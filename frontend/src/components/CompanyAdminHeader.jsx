import { Search, Menu, X, Briefcase, Zap } from 'lucide-react'
import { useState } from 'react'
import UserMenu from './UserMenu'
import NotificationBell from './NotificationBell'

const CompanyAdminHeader = ({ title, subtitle, showSearch = false, onMenuToggle, isMobileMenuOpen = false }) => {
  const [searchQuery, setSearchQuery] = useState('')

  return (
    <header className="sticky top-0 z-40 backdrop-blur-xl bg-gradient-to-r from-slate-950/95 via-slate-900/95 to-slate-950/95 border-b border-blue-500/20 shadow-lg shadow-blue-500/5">
      ...
      <div className="px-3 md:px-5 lg:px-6 py-1.5 md:py-1.5 lg:py-2">
        <div className="flex items-center justify-between gap-4">
          ...
          {showSearch && (
            <div className="hidden md:flex flex-1 max-w-md">
              <div className="relative w-full group">
                <div className="absolute inset-0 bg-gradient-to-r from-blue-500/20 to-cyan-500/20 rounded-lg blur opacity-0 group-focus-within:opacity-100 transition-all duration-300"></div>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2 bg-slate-900/50 border border-blue-500/20 focus:border-blue-500/60 rounded-lg text-sm text-white placeholder:text-slate-500 font-mono transition-all duration-200 focus:shadow-lg focus:shadow-blue-500/10"
                  />
          ...
          <div className="flex items-center gap-1 md:gap-2">
            ...
            <NotificationBell />
            <div className="hidden sm:block h-8 w-px bg-gradient-to-b from-transparent via-blue-500/30 to-transparent" />
            <UserMenu />
          </div>
        </div>
          ...
          <div className="md:hidden mt-3">
            <div className="relative group">
              ...
              <input
                ...
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search operations..."
                className="w-full pl-10 pr-4 py-2 bg-slate-900/50 border border-blue-500/20 focus:border-blue-500/60 rounded-lg text-sm text-white placeholder:text-slate-500 font-mono transition-all duration-200"
              />
            </div>
          </div>
        )}
