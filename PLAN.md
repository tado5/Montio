...
- **Používateľ DB:** u46895_montio / [See Hostcreator ENV_FILE]
- **Hosting:** Hostcreator
- **Auth:** JWT + bcrypt
- **PDF:** jsPDF + QR kódy
- **Grafy:** Recharts
...
- **DB Connection:** See backend/.env or Hostcreator ENV_FILE
...
## FÁZA 5: ZÁKAZKY WIZARD (CORE) ✅ 95% HOTOVO (2026-04-17)

### Order Timeline / Logging System 📝

**Účel:** Transparentná história zákazky pre všetkých - klient vidí čo sa deje, zamestnanec má prehľad, company admin kontroluje proces.

**Princíp: KTO? ČO? KEDY?**

Každá akcia na zákazke sa zaloguje do `order_timeline` tabuľky:
...
- `quote_sent` - Cenová ponuka odoslaná
- `quote_accepted` - Ponuka akceptovaná klientom
- `quote_rejected` - Ponuka zamietnutá
- `employee_assigned` - Priradený zamestnanec
- `installation_started` - Začala montáž
- `installation_completed` - Montáž dokončená
- `invoice_generated` - Faktúra vygenerovaná
- `payment_received` - Platba prijatá
- `order_cancelled` - Zákazka zrušená
- `note_added` - Pridaná poznámka
- `photo_uploaded` - Nahratá fotka
- `document_uploaded` - Nahratý dokument (PDF protokol)
...
```json
{
  ...
  "files": ["protokol_obhliadka_2024_001.pdf"],
...
**API Endpoints:**
```javascript
// GET /api/orders/:id/timeline - Načítanie timeline zákazky
// POST /api/orders/:id/timeline - Pridanie záznamu (systém alebo manuálne)
// GET /api/orders/client/:clientToken/timeline - Timeline pre klienta (cez unique link)
```

**Frontend Timeline Component:**
```jsx
<OrderTimeline orderId={123}>
  {/* Vertikálna timeline s ikonami */}
  <TimelineItem
    icon={<CheckCircle />}
    title="Obhliadka dokončená"
    user="Ján Novák (Technik)"
    date="13.4.2024 14:30"
    details="2 fotky nahrané, protokol vytvorený"
    files={["protokol.pdf"]}
  />
  <TimelineItem
    icon={<FileText />}
    ...
    user="Systém"
    date="13.4.2024 15:00"
    details="Cena: 1 250,00 € (s DPH)"
    badge="Čaká sa na odpoveď"
  />
  {/* ... */}
</OrderTimeline>
```
...
```
- Unikátny token (UUID) pre každú zákazku
- Klient vidí len záznamy s `visible_to_client = TRUE`
- Nevidí ceny interných nákladov, marže, poznámky pre zamestnancov
- Vidí fotky, PDF protokoly, status, termíny

**Automatické logovanie:**
Každá zmena na zákazke sa automaticky zaloguje cez middleware:
```javascript
// backend/middleware/orderLogger.js
export const logOrderAction = async (orderId, userId, actionType, title, details, visibleToClient = true) => {
  await pool.query(
    `INSERT INTO order_timeline (order_id, user_id, action_type, action_title, action_details, visible_to_client)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [orderId, userId, actionType, title, JSON.stringify(details), visibleToClient]
  );
};
```

**Real-time notifikácie:**
- Klient dostane email/SMS pri každom zázname
- Zamestnanec dostane notifikáciu pri pridelení
- Company admin vidí všetky zmeny v realtime
...
- ✅ Survey: POST, PUT `/api/orders/:id/survey`
- ✅ Quote: POST, PUT `/api/orders/:id/quote` + quote_link
- ✅ Installation: POST, PUT `/api/orders/:id/installation`
- ✅ Completion: POST, PUT `/api/orders/:id/completion`
- ✅ Public Quote: GET, POST `/api/orders/public/quote/:quoteLink`

**Frontend Pages (8 súborov):**
- ✅ CreateOrderPage, OrdersPage, OrderDetailPage
- ✅ SurveyStagePage, QuoteStagePage, InstallationStagePage, CompletionStagePage
- ✅ ClientQuoteViewPage (verejná stránka bez auth)

**Database Migrations:**
- ✅ 8 nových stĺpcov (orders + order_stages)
- ✅ Production migration deployed (phpMyAdmin)

**Features:**
- ✅ 4-stage workflow: Survey → Quote → Installation → Completion
- ✅ Photo upload (base64) pre všetky stages
- ✅ Checklist completion tracking
- ✅ Company & client signatures (react-signature-canvas)
- ✅ Public quote page s unique link
- ✅ Client signature workflow + notifications
- ✅ Activity timeline zobrazenie

**Zostáva (5% refactoring - neblokujúce):**
- ⏳ Integrovať useSignature/usePhotoUpload hooks
- ⏳ LoadingSpinner component extraction
...
### 5-krokový OrderWizard (React Hook Form + Zustand) - PÔVODNÝ PLÁN
...
| ✅ FÁZA 5 | Zákazky Wizard | **95% HOTOVO** | ⭐ Orders CRUD, 4 Stage Pages, Public Quote, Client Signatures (2026-04-17) |
| 🎯 FÁZA 6 | Fakturácia | **ĎALŠÍ KROK** | PDF + QR kódy |
...
### Remember Me Feature (v1.11.2)
- **Checkbox na login page:** "Zapamätať si ma"
- **localStorage:** Ukladá email + password (plain text)
- **Auto-fill:** Pri návrate na stránku sú údaje vyplnené
- **Per-device storage:** Každé zariadenie/browser má vlastné údaje
- **⚠️ Security:** Plain text storage - **len pre testovanie**, nie produkciu
- **Use case:** Multi-device testing (mobile, tablet, desktop)
- **Implementácia:** `frontend/src/pages/Login.jsx` (3 localStorage keys)
